document.addEventListener('DOMContentLoaded', () => {
  ...
  const SPLIT_CONFIG = { type: 'words, chars', wordsClass: 'concept-word', charsClass: 'concept-char' };
  const buildRevealTargets = (splitInstance) => {
    const targets = [];
    splitInstance.words.forEach((word) => {
      if (CJK_PATTERN.test(word.textContent)) {
        word.style.display = 'inline';
        word.querySelectorAll('.concept-char').forEach((char) => targets.push(char));
      } else {
        targets.push(word);
      }
    });
  ...
  let split = new SplitText(heading, SPLIT_CONFIG);
  let revealEls = buildRevealTargets(split);
  ...
  ScrollTrigger.create({
    ...
    onUpdate: (self) => {
      ...
      if (pct >= WORD_TRIGGER && !wordsRevealed) {
        ...
        wordsTween = gsap.to(revealEls, { opacity: 1, y: 0, duration: 0.3, stagger: 0.02, ease: 'power2.out' });
      }
      if (pct < WORD_TRIGGER && wordsRevealed) {
        wordsRevealed = false;
        if (wordsTween) wordsTween.kill();
  ...
  window.refreshConceptText = () => {
    split.revert();
    split = new SplitText(heading, SPLIT_CONFIG);
    revealEls = buildRevealTargets(split);
    gsap.set(revealEls, { opacity: 0, y: 20 });
