!function () {
  ...
  var LENIS_OPTS = { duration: 2, easing: function (t) { return t === 1 ? 1 : 1 - Math.pow(2, -10 * t); }, smoothWheel: true, smoothTouch: false };
  ...
  var descriptionText = overlay.querySelector('.popup--content-right .small-text');
  ...
  var newsTitle = overlay.querySelector('.news--lightbox-content .heading-small');
  ...
  var popupIngredientImg = (function () { var el = overlay.querySelector('[data-popup="ingredient-image"]'); if (el) el.removeAttribute('loading'); return el; })();
  ...
  var popupDescription = overlay.querySelector('[data-popup="full-description"]');
  ...
  function lenisLoop(time) {
    lenisInstance && (lenisInstance.raf(time), lenisRaf = requestAnimationFrame(lenisLoop));
  }
  ...
  function createPopupLenis(wrapper) {
    ...
    wrapper.scrollTop = 0;
    lenisInstance = new Lenis(Object.assign({ wrapper: wrapper, content: wrapper.children[0] }, LENIS_OPTS));
  ...
  function blockScroll(ev) {
    ...
    if (mode === 'product') {
      var node = ev.target;
      while (node && node !== document) {
        if (node.classList && node.classList.contains('popup--content-right')) return;
        node = node.parentNode;
      }
    }
    ev.preventDefault();
  ...
  function createTexture(src, callback, storePreload) {
    ...
    img.onload = () => {
      ...
      gl.bindTexture(gl.TEXTURE_2D, tex);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, gl.RGBA, gl.UNSIGNED_BYTE, fitted);
  ...
  function renderWebGL() {
    ...
    var canvasAspect = webglCanvas.width / ZOOM_SCALE / (webglCanvas.height / ZOOM_SCALE);
    var texInfo = webglState.textureInfos.current;
    var texAspect = texInfo.width / texInfo.height;
    var sx = 1, sy = 1;
    canvasAspect > texAspect ? sy = texAspect / canvasAspect : sx = canvasAspect / texAspect;
    sx *= ZOOM_SCALE;
    sy *= ZOOM_SCALE;
    m4.scale(texMatrix, [sx, sy, 1], texMatrix);
    webglState.time++;
    m4.ortho(0, webglCanvas.width, webglCanvas.height, 0, -1, 1, projection);
    m4.translate(projection, [webglCanvas.width / 2, webglCanvas.height / 2, 1], projection);
    m4.scale(projection, [webglCanvas.width, webglCanvas.height, 1], projection);
    gl.useProgram(programInfo.program);
    twgl.setBuffersAndAttributes(gl, programInfo, bufferInfo);
    twgl.setUniforms(programInfo, { uMatrix: projection, uTmatrix: texMatrix, uTexOne: webglState.textures.current, uTexTwo: webglState.textures.next, uTime: webglState.time, uPower: webglState.power, uWipeProgress: webglState.wipeProgress, uOffset: webglState.offset });
  ...
  function cleanupWebGL() {
    stopRendering();
    webglState.textures.current && (gl.deleteTexture(webglState.textures.current), webglState.textures.current = null);
    webglState.textures.next && (gl.deleteTexture(webglState.textures.next), webglState.textures.next = null);
  ...
  function runTransition(currentSrc, nextSrc, altText, onComplete) {
    ...
    var onTextureReady = () => {
      ...
      rippleTl.to(webglState, { power: 0.3, duration: 0.4, ease: 'none' }).to(webglState, { power: 0, duration: 0.8, ease: 'none' });
      gsap.timeline({
        onComplete: () => {
          ...
          galleryImg1.decode ? galleryImg1.decode().then(finish).catch(finish) : requestAnimationFrame(finish);
        },
      }).to(rippleTl, { progress: 1, duration: 1.5, ease: 'power2.inOut' }, 0).to(webglState, { wipeProgress: 1, duration: 1, ease: 'none' }, 0.5);
    };
    webglState.textures.current = createTexture(currentSrc, (info) => { webglState.textureInfos.current = info; onTextureReady(); }, false);
    webglState.textures.next = createTexture(nextSrc, (info) => { webglState.textureInfos.next = info; onTextureReady(); }, true);
  }
  ...
  var products = Array.from(productWrappers).map((el) => {
    ...
    return {
      ...
      perfumeName: el.dataset.perfumeName || el.querySelector('.product-title')?.textContent || '',
  ...
  function preloadImage(url) {
    ...
    return new Promise(function (resolve) {
      var img = new Image();
      img.src = url;
      var done = function () { imageCache.set(url, true); resolve(); };
      img.decode ? img.decode().then(done).catch(done) : (img.onload = done, img.onerror = done);
    });
  }
  ...
  function resetSlideLines() {
    slideLines.forEach((line) => {
      ...
      if (fill) { gsap.killTweensOf(fill); gsap.set(fill, { x: '-100%' }); }
    });
  }
  ...
  function goToSlide(idx, product, immediate) {
    ...
    sliding = true;
    gsap.delayedCall(0.75, () => { resetSlideLines(); startAutoplay(idx, product); });
    runTransition(prevSrc, nextSrc, product.name, () => { sliding = false; });
  }
  ...
  function startAutoplay(idx, product) {
    ...
    if (line && product.gallery.length > 1) {
      var fill = line.querySelector('.slide--line-fill');
      if (fill) {
        autoplayTween = gsap.fromTo(fill, { x: '-100%' }, {
          x: '0%', duration: AUTOPLAY_DUR, ease: 'linear', onComplete: () => {
            if (!sliding) goToSlide((slideIndex + 1) % product.gallery.length, product);
          },
        });
      }
    }
  ...
  function populateProduct(product) {
    galleryImg1 && (galleryImg1.src = product.gallery[0], galleryImg1.alt = product.name, gsap.set(galleryImg1, { opacity: 1 }));
    ...
    if (popupIngredientImg) {
      ...
      if (product.ingredientImage) {
        if (imageCache.has(product.ingredientImage)) {
          ...
          preloadImage(src).then(function () { popupIngredientImg.src = src; popupIngredientImg.alt = alt; });
        }
      }
    }
    ...
    popupDescription && (popupDescription.textContent = product.fullDescription);
    ...
    slideLines.forEach((line, i) => line.style.display = i < count ? '' : 'none');
    resetSlideLines();
  }
  ...
  function initWebGL() {
    ...
    webglCanvas.style.cssText = 'position:absolute;top:-30%;left:-30%;width:160%;height:160%;z-index:1;pointer-events:none;opacity:0';
    galleryWrapper.insertBefore(webglCanvas, galleryWrapper.querySelector('.gallery-slide'));
    gl = webglCanvas.getContext('webgl', { alpha: true, premultipliedAlpha: false, antialias: true });
    if (!gl) { webglCanvas.remove(); return false; }
    try {
    ...
    } catch (err) { webglCanvas.remove(); return false; }
    webglReady = true;
    return true;
  }
  ...
  function openPopup(popupMode, index) {
    ...
    if (popupMode === 'product') {
      ...
      if (newsImage) { newsImage.style.display = item.image ? '' : 'none'; newsImage.src = item.image || ''; newsImage.alt = item.title; }
      newsBody && (newsBody.innerHTML = item.body);
      overlay.classList.add('show-news');
    }

    content && (content.style.display = 'grid', content.style.opacity = '0', content.scrollTop = 0);
    ...
    if (popupMode === 'product' && closeBtn && window.innerWidth <= MOBILE_BREAKPOINT) closeBtn.style.filter = 'invert(1)';
    typeof SScroll !== 'undefined' ? SScroll.stop() : (document.documentElement.style.overflow = 'hidden', document.body.style.overflow = 'hidden');
    ...
    overlay.style.transition = 'opacity 800ms ' + EASE_OVERLAY + ',visibility 800ms ' + EASE_OVERLAY;
    overlay.classList.add('is-active');

    setTimeout(() => {
      overlay.style.transition = '';
      content && (content.style.transition = 'opacity 700ms ' + EASE_CONTENT, content.style.opacity = '1');
      closeBtn && (closeBtn.style.transition = 'opacity 700ms ' + EASE_CONTENT, closeBtn.style.opacity = '1');
  ...
  function closePopup() {
    ...
    overlay.style.transition = 'opacity 700ms ' + EASE_CONTENT + ',visibility 700ms ' + EASE_CONTENT;
    ...
    setTimeout(() => {
      ...
      content && (content.style.transition = '', content.style.opacity = '', content.style.display = '');
      closeBtn && (closeBtn.style.transition = '', closeBtn.style.opacity = '', closeBtn.style.filter = '');
      typeof SScroll !== 'undefined' ? SScroll.start() : (document.documentElement.style.overflow = '', document.body.style.overflow = '');
  ...
  function switchProduct(newIndex) {
    ...
    var preloads = [preloadImage(nextData.ingredientImage), preloadImage(nextData.gallery[0])];

    productContent && (productContent.style.transition = 'opacity 700ms ' + EASE_CONTENT, productContent.style.opacity = '0');
    closeBtn && (closeBtn.style.transition = 'opacity 700ms ' + EASE_CONTENT, closeBtn.style.opacity = '0');

    var fadeOutDone = new Promise(function (resolve) { setTimeout(resolve, CONTENT_DELAY); });
    ...
    Promise.all([fadeOutDone].concat(preloads)).then(function () {
      ...
      var showContent = function () {
        requestAnimationFrame(function () {
          requestAnimationFrame(function () {
            productContent && (productContent.style.transition = 'opacity 700ms ' + EASE_CONTENT, productContent.style.opacity = '1');
            closeBtn && (closeBtn.style.transition = 'opacity 700ms ' + EASE_CONTENT, closeBtn.style.opacity = '1');
          });
        });
      };
      galleryImg1 && galleryImg1.decode ? galleryImg1.decode().then(showContent).catch(showContent) : showContent();
      setTimeout(function () {
  ...
  productWrappers.forEach((el, i) => { el.style.cursor = 'pointer'; el.addEventListener('click', () => openPopup('product', i)); });
  newsRows.forEach((el, i) => { el.style.cursor = 'pointer'; el.addEventListener('click', () => openPopup('news', i)); });
  ...
  if (galleryWrapper && window.innerWidth <= MOBILE_BREAKPOINT) {
    var touchStartX = 0;
    galleryWrapper.addEventListener('touchstart', (ev) => { touchStartX = ev.touches[0].clientX; }, { passive: true });
    galleryWrapper.addEventListener('touchend', (ev) => {
      if (transitioning || sliding || mode !== 'product') return;
      var dx = ev.changedTouches[0].clientX - touchStartX;
      if (Math.abs(dx) < SWIPE_THRESHOLD) return;
      var product = products[productIndex];
      if (!product || product.gallery.length <= 1) return;
      var next = dx < 0 ? (slideIndex + 1) % product.gallery.length : (slideIndex - 1 + product.gallery.length) % product.gallery.length;
      if (next !== slideIndex) { autoplayTween && (autoplayTween.kill(), autoplayTween = null); goToSlide(next, product); }
    }, { passive: true });
  }
  caretLeft?.addEventListener('click', (ev) => { ev.stopPropagation(); switchProduct((productIndex - 1 + products.length) % products.length); });
  caretRight?.addEventListener('click', (ev) => { ev.stopPropagation(); switchProduct((productIndex + 1) % products.length); });
  ...
  newsItems.forEach(function (item, i) {
    if (item.title.toLowerCase() === 'privacy policy') {
      privacyIndex = i;
      var row = newsRows[i];
  ...
  var privacyBtn = document.querySelector('.footer-content [data-privacy]') || document.querySelector('[data-privacy]');
  if (privacyBtn && privacyIndex >= 0) {
    privacyBtn.style.cursor = 'pointer';
    privacyBtn.addEventListener('click', function (ev) { ev.preventDefault(); openPopup('news', privacyIndex); });
  }

  window.lightbox = { openProduct: (i) => openPopup('product', i), openNews: (i) => openPopup('news', i), close: closePopup, nextProduct: () => switchProduct((productIndex + 1) % products.length), prevProduct: () => switchProduct((productIndex - 1 + products.length) % products.length) };
}();
