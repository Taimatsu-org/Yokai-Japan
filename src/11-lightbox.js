!function () {
    ...
    if (popupMode === 'product') {
      ...
      if (newsImage) {
        ...
        if (item.image) {
          ...
          preloadImage(item.image).then(function () { newsImage.src = item.image; newsImage.alt = item.title; });
        } else {
  ...
  FOOTER_NEWS_LINKS.forEach(function (link) {
    ...
    if (index < 0) return;
    var btn = document.querySelector('.footer-content [' + link.attr + ']') || document.querySelector('[' + link.attr + ']');
    if (!btn) return;
    btn.style.cursor = 'pointer';
    btn.addEventListener('click', function (ev) { ev.preventDefault(); openPopup('news', index); });
  });
