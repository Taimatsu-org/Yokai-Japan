!(function () {
  ...
  var caretLeft = overlay.querySelectorAll(".popup-caret.left");
  var caretRight = overlay.querySelectorAll(".popup-caret.right");
  ...
  var products = Array.from(productWrappers).map((el) => {
    ...
    return {
      ...
      variant: el.dataset.variant || el.dataset.size || "",
    };
  });
  ...
  function populateProduct(product) {
    ...
    var mTitles = overlay.querySelectorAll(
      "[data-modal-buy] [data-shopify-title]",
    );
  ...
  function siblingIndex(dir) {
    var group = products[productIndex].variant;
    var i = productIndex;
    for (var step = 0; step < products.length; step++) {
      i = (i + dir + products.length) % products.length;
      if (products[i].variant === group) return i;
    }
    return productIndex;
  }

  caretLeft.forEach((el) => {
    el.addEventListener("click", (ev) => {
      ev.stopPropagation();
      var idx = siblingIndex(-1);
      if (idx !== productIndex) switchProduct(idx);
    });
    el.style.cursor = "pointer";
  });
  caretRight.forEach((el) => {
    el.addEventListener("click", (ev) => {
      ev.stopPropagation();
      var idx = siblingIndex(1);
      if (idx !== productIndex) switchProduct(idx);
  ...
  function applyVariant(v, mBtns, mPrices) {
    ...
    mBtns.forEach(function (btn) {
      ...
      btn.textContent = v.available ? t.addToCart : t.soldOut;
    });
  }
