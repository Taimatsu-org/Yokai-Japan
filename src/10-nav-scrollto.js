document.addEventListener('DOMContentLoaded', () => {
  ...
  var getSectionId = (link) => {
    var text = link.querySelector('.small-text')?.textContent.trim().toLowerCase();
    if (text && text.length % 2 === 0 && text.slice(0, text.length / 2) === text.slice(text.length / 2)) text = text.slice(0, text.length / 2);
